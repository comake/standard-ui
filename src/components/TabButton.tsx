...
export function TabButton<T>({
    ...
    const pendingUI = useMemo(() => <CircularProgress className="ml-core-dimension-25 !size-core-dimension-75" />, []);

    if ((differentiatorProps as TabLinkButtonProps).link) {
        const tabLinkProps = differentiatorProps as TabLinkButtonProps;

        return (
            <Link href={tabLinkProps.link} className={className} onClick={handleClickLink}>
                {label}
                {isPending && pendingUI}
            </Link>
        );
